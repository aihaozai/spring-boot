...
layui.extend({
    cols: 'lay/modules/cols'
}).define(['laytpl', 'laypage', 'layer', 'form', 'util','cols'], function(exports){
  "use strict";
  ...
  Class.prototype.events = function(){
    ...
    //自定义单元格双击事件
    that.layBody.on('dblclick', 'td', function(e){
      var othis = $(this)
      ...
      if(othis.data('off')) return; //不触发事件
      if(customize) { //动态根据其他单元格值是否显示编辑表单
          if(!cols.checkCols(othis,customize)) return;
      }
        //显示编辑表单
      if(editType=="dblEdit"){
